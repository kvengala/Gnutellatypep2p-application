/*pa 3 by vengalathur srikanth Kashyap and Vijai Bhaskar
 * 
 * 
 */
	...
	public static void main (String args[]) throws Exception
	{
		...
		do
			...
			System.out.println("Select from the follwoing options"
				...
				+ "\n 3.Search for a file in other peers");
	   ...
	   choose = scr.nextInt();
		...
		case 3:
			...
			Registry regy2 = LocateRegistry.getRegistry(property.getProperty("IPAddress"),Integer.parseInt(property.getProperty("portid")));
			rmi_interface lookup2 = (rmi_interface) regy2.lookup(property.getProperty("PeerName"));
			lookup2.left_traversal(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			lookup2.right_traversal(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			lookup2.left_traversal_two(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			lookup2.right_traversal_two(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			lthshmp = lookup2.left_peernm(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			lthshmp_two = lookup2.left_peernm_two(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			rgthshmp = lookup2.right_peernm(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			rgthshmp_two = lookup2.right_peernm_two(property.getProperty("PeerName"), Integer.parseInt(property.getProperty("portid")));
			int pnu = Integer.parseInt(property.getProperty("portid"));
			System.out.println("Enter a file to be searched");
			String filename = br.readLine();
			String [] valq = property.getProperty("PeerName").split("-");
					...
					if (con == true)
			{
				...
				String fullreturnhname = lookups.ret_conts();
				String[]fullpathname = fullreturnhname.split("=");
			...
			val1 =  Integer.parseInt(valq[1]);
			pnu = Integer.parseInt(property.getProperty("portid"));
			for ( int iter = pnu+1; iter <= rthsm ; iter++ ) 
			{
		...
		System.out.println("The time taken for the download is " + timetaken/1e6 + "milliseconds");
			break;		
    	...
    	public void run()
    	{
			...
			for(int i=0;i< filelist3.length;i++)
			{
				...
				int pij =  Integer.parseInt(val[1]);
				 valq = prop.getProperty("PeerName").split("-");
				int val1 =  Integer.parseInt(valq[1]);
